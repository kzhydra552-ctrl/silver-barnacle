...
async function registerCommands() {
  ...
  const guildIds = [process.env.GUILD_ID_1, process.env.GUILD_ID_2, process.env.GUILD_ID]
    .map(id => id?.trim())
    .filter(Boolean);
  if (!CLIENT_ID || guildIds.length === 0) {
    console.warn('CLIENT_ID e GUILD_ID_1/GUILD_ID_2 são necessários para registrar os comandos.');
    return;
  }
  const commands = [
  ...
  console.log(`Comandos slash registrados em ${guildIds.length} servidor(es).`);
}
