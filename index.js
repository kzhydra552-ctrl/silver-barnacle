...
function panelEmbed() {
  const embed = new EmbedBuilder()
    ...
    .addFields({ name: 'Como usar', value: 'Use **Key grátis** para receber uma chave gratuita ou clique em **Resgatar chave** para informar uma chave. Depois, use **Ver conteúdo** para receber a entrega autorizada por DM.' })
...
async function registerCommands() {
  ...
  const commands = [
    ...
    new SlashCommandBuilder().setName('config').setDescription('Configura o painel').addStringOption(o => o.setName('descricao').setDescription('Descrição').setRequired(false)).addStringOption(o => o.setName('conteudo').setDescription('Conteúdo autorizado enviado por DM').setRequired(false)).addStringOption(o => o.setName('thumbnail').setDescription('URL da thumbnail; deixe vazio para remover').setRequired(false)),
